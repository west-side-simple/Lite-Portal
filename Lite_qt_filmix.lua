...
function run_lite_qt_filmix()
	...
	local tt = {
		{"filmi","Жанровое кино"},
		{"seria","Жанровые сериалы"},
		{"mults","Жанровые мульты"},
		{"multserialy","Жанровые мультсериалы"},
...
function ganres_content_filmix(url)
	local title
	if url:match('filmi/') then title = 'Жанровое кино'
	elseif url:match('seria/') then title = 'Жанровые сериалы'
	elseif url:match('multserialy/') then title = 'Жанровые мультсериалы'
	elseif url:match('mults/') then title = 'Жанровые мульты'
	end
...
---------------
	...
	t.ExtParams = {FilterType = 1, AutoNumberFormat = '%1. %2'}
		local ret,id = m_simpleTV.OSD.ShowSelect_UTF8('Выберите коллекцию Filmix (' .. #t .. ') ' .. title,0,t,10000,1+4+8+2)
		if ret == -1 or not id then
			return
		end
		if ret == 1 then
...
---------------
		...
		answer = answer:match('<div class="clr playlist%-articles.-<div class="navigation">') or ''
		for w in answer:gmatch('<article.-<div class="panel') do
		local adr,logo,name,desc
		adr = w:match('itemprop="url" href="(.-)"') or ''
		logo = w:match('<img src="(.-)"') or ''
