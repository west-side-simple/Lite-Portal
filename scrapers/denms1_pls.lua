-- скрапер TVS для загрузки плейлиста "DENMS TV - tsn" http://denms.tplinkdns.com/ (29/03/22)
...
-- ##
	...
	module('denms1_pls', package.seeall)
	local my_src_name = 'DENMS TV - tsn (A)'
	...
	local function LoadFromSite(pls, pls_name)
		...
		{"http://ua.iptvin.ru/?","http://ua.iptvin.ru/?"},
		{"http://sh3304149.c.had.su/?","http://sh3304149.c.had.su/?"},
		{"https://sh3304149.c.had.su/?","https://sh3304149.c.had.su/?"},
}
  local t1={}
  for i=1,#zer do
    t1[i] = {}
  ...
  if ret == 1 then
   ...
   cur_zer = t1[id].Action
  end

		url = cur_zer .. pls .. '&' .. login .. '&' .. password
		end
