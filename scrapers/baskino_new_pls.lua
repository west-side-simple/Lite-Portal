	...
	module('baskino_new_pls', package.seeall)
	local my_src_name = 'BASKINO TOP'
...
local background = ''
	...
	local rc, answer = m_simpleTV.Http.Request(session, {url = url})
	if rc ~= 200 then return end
	adr = answer:match('data%-sign="vb" data%-url="(.-)"') or ''
	name = answer:gsub('\n',''):match('<h1>(.-)</h1>') or ''
	logo = adr:match('embed/(tt%d+)%,')
	if logo then logo = bg_imdb_id(logo) else logo = '' end
	adr = getadr1(adr)
...
--		m_simpleTV.Http.Close(session)
		...
		answer = answer:match('<div class="carousel">(.-)</ul>')
		local t, i = {}, 1
			for w in answer:gmatch('<li>.-</li>') do
			local adr, name, logo = w:match('<a href="(.-)"><img title="(.-)" src="(.-)"')
...
if adr:match('/films/fantasticheskie/') then t[i].group = 'Фантастические' end
				...
				t[i].address, t[i].name, t[i].logo =  getadr(url .. adr)
