-- скрапер TVS для загрузки плейлиста "FoxTV" http://pa.fox-tv.fun/ (19/5/21)
...
-- ##
	...
	module('fox_pls', package.seeall)
	local my_src_name = 'FoxTV ($)'
	...
	function GetSettings()
	 return {name = my_src_name, sortname = '', scraper = '', m3u = 'out_' .. my_src_name .. '.m3u', logo = '../Channel/logo/icons/foxtv.png', TypeSource = 1, TypeCoding = 1, DeleteM3U = 1, RefreshButton = 1, AutoBuild = 0, AutoBuildDay = {0, 0, 0, 0, 0, 0, 0}, LastStart = 0, TVS = {add = 0, FilterCH = 1, FilterGR = 1, GetGroup = 1, LogoTVG = 1}, STV = {add = 1, ExtFilter = 1, FilterCH = 1, FilterGR = 1, GetGroup = 1, HDGroup = 0, AutoSearch = 1, AutoNumber = 0, NumberM3U = 0, GetSettings = 0, NotDeleteCH = 0, TypeSkip = 1, TypeFind = 1, TypeMedia = 0, RemoveDupCH = 0}}
	end
	function GetVersion()
	 return 2, 'UTF-8'
	end
	local function LoadFromSite()
		...
		m_simpleTV.OSD.ShowMessageT({imageParam = 'vSizeFactor="1.0" src="' .. m_simpleTV.Common.GetMainPath(2) .. './work/Channel/logo/icons/foxtv.png"', text = ' Внесите данные в менеджере паролей для ID FOX', color = ARGB(255, 255, 255, 255), showTime = 1000 * 60})
		else 
		url = 'http://pl.fox-tv.fun/' .. login
					.. '/' .. password
					.. '/tv.m3u'
		end
