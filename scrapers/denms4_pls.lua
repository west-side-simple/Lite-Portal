-- скрапер TVS для загрузки плейлиста "DENMS TV - media" http://denms.tplinkdns.com/ (27/07/21)
...
-- ##
	...
	module('denms4_pls', package.seeall)
	local my_src_name = 'DENMS TV - media (A)'
	...
	local function LoadFromSite(pls, pls_name)
		...
		local t, i = {}, 1
			...
			for w in answer:gmatch('%#EXTINF:.-\n.-\n') do
				...
				t[i].name = title
				t[i].address = adr
				t[i].group = w:match('group%-title="([^"]+)') or 'DENMS TV - ужастики'
				if logo then
				t[i].logo = logo
				else
				t[i].logo = 'http://m24.do.am/images/mix.png'
				end
				if (t[i].group:match('Союзмультфильм') or t[i].group:match('Мультфильмы')) and pls_name:match('Мультфильмы') or t[i].group:match('Фильмотека') and pls_name:match('Фильмы') or t[i].group:match('DENMS') and t[i].name:match('^.- %((%d+)%)') and pls_name:match('media') or (t[i].group:match('Союзмультфильм') or t[i].group:match('Мультфильмы')) and pls_name:match('media') or t[i].group:match('Фильмотека') and pls_name:match('media') or t[i].group:match('DENMS') and t[i].name:match('^.- %((%d+)%)') and pls_name:match('media') then
				i = i + 1
				end
			end
	...
	function GetList(UpdateID, m3u_file)
		...
		local pll={
		{"web3-a","DENMS TV - media, полная версия"},
		{"web3-a","Мультфильмы"},
		{"web3-a","Фильмы"},
}
