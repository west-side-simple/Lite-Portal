-- скрапер TVS для балансера Vibix от west_side (22/05/25)
...
-- ## Переименовать группы ##
	...
	module('VFfilm_WS_pls', package.seeall)
	local my_src_name = 'VFFILM WS'	
	...
	local function LoadFromPage()
		...
		while page <= page_max do
			...
			if rc ~= 200 or not answer:match('"id"') then break end
			if tonumber(page) == 0 then Clean_VF() end
			str = str .. unescape3(answer):gsub('\n', ' '):gsub('"\\"', '"«'):gsub('\\"', '»')
			page = page + 1
			m_simpleTV.OSD.ShowMessageT({text = 'VFFILM : (стр. ' .. (tonumber(page)) .. ' из ' .. (tonumber(page_max+1)) .. ')' ,0xFF00,3})
		end
		return str
	end
	...
	local function LoadFromSite(variant)
		...
		if not answer then return end
			for w in answer:gmatch('%{(.-)"tags":.-%[.-%]') do
				...
				genre_title = genre:gsub('^комедия%, драма%, ',''):gsub('^комедия%, ',''):gsub('^драма%, ',''):gsub('%,.-$','')
				country = w:match('"country":.-"(.-)"') or ''
				country_title = country:gsub('%,.-$','')
				if country_title == '' then country_title = 'нетфлага' end
				--костыли
				if tonumber(adr) == 28660 then genre_title = 'ужасы' end
				if tonumber(adr) == 21694 then year = '2024' end
				if tonumber(adr) == 30913 then year = '2024' end
				if tonumber(adr) == 30893 then year = '2023' end
				if tonumber(adr) == 53517 then year = '2024' end
				if tonumber(adr) == 41373 then year = '2024' end
				if tonumber(adr) == 86145 then year = '2025' end
				if tonumber(adr) == 87743 then year = '2025' end
				if tonumber(adr) == 87358 then year = '2024' end
				if tonumber(adr) == 92261 then year = '2025' end
				if tonumber(adr) == 91132 then year = '2025' end
				if tonumber(adr) == 90522 then year = '2025' end
				if tonumber(adr) == 87730 then year = '2024' end
				...
				if name and adr then
					...
					if tonumber(adr) ~= 37290 then -- костыль
						t[i] = {}
						...
						if country:match('Россия') and tonumber(variant) ~= 2 then
							suf = '.png'
							if genre_title == 'мультфильм' then
								t[i].group = 'Российские мультфильмы'
							else
								t[i].group = 'Российские фильмы'
							end
						end
						if country:match('СССР') and tonumber(variant) ~= 2 then
							suf = '.png'
							if genre_title == 'мультфильм' then
								t[i].group = 'Советские мультфильмы'
							else
								t[i].group = 'Советские фильмы'
							end
						end
						t[i].logo = './luaScr/user/show_mi/pause/Vibix.png'
						t[i].name = name:gsub('\\u2013', '-'):gsub('\\u2014', '-'):gsub('\\u2026', '...'):gsub('\\u00ab', '«'):gsub('\\u00bb', '»'):gsub('\\u2116', '№'):gsub('\\u2019', '`'):gsub('\\/', '/') .. ' (' .. year .. ')'
						t[i].address = 'https://672723821.videoframe1.com/embed/' .. adr .. embed
						t[i].video_title = data_pub .. ' | ' .. genre .. ' | ' .. country
	...
	function GetList(UpdateID, m3u_file)
		...
		local playlist
		local ret,id = m_simpleTV.OSD.ShowSelect_UTF8('VibixFilm - select playlist',0,t,9000,1+4+8)
