-- скрапер TVS для загрузки плейлиста "Фрегат ТВ" https://fregat.com (12/1/21)
-- Copyright © 2017-2021 Nexterr | https://github.com/Nexterr/simpleTV
-- UP for catchup - westSide (06/08/22)
...
-- ##
	...
	local function LoadFromSite()
		...
		local t, i = {}, 1
		local extOpt = decode64('JE9QVDpodHRwLWV4dC1oZWFkZXI9eC1zbWFydGxhYnMtbWFjLWFkZHJlc3M6RjU6MTE6QTA6MEY6MjI6RkY')
		local logoHost = url:match('^https?://[^/]+')
			for w in answer:gmatch('<channel>.-</channel>') do
				...
				local arch = w:match('<tstvOttURL>([^<]+)')
				if adr and name then
					t[i] = {}
					t[i].name = name:gsub('amp;', '')
					t[i].address = adr .. extOpt
					if arch and arch~='' then
					t[i].RawM3UString = 'catchup="default" catchup-days="7" catchup-source="' ..arch ..'?pauselive=&utcstart=${start}&closefull=&reason=seek"'
					end
