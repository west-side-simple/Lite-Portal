--kinokong portal - lite version west_side 01.04.25
...
function run_lite_qt_kinokong()
	...
	local last_adr = getConfigVal('info/kinokong') or ''
		...
		{"","Подборки KinoKong"},
		{"","Актёры"},
		{"/kino/","Фильмы - последние поступления"},
		{"/kino/boeviky/","Боевик"},
		{"/kino/vestern/","Вестерн"},
		{"/kino/voennyy/","Военный"},
		{"/kino/detektiv/","Детектив"},
		{"/kino/drama/","Драма"},
		{"/kino/istoricheskiy/","Исторический"},
		{"/kino/komedian/","Комедия"},
		{"/kino/kriminal/","Криминал"},
		{"/kino/melodrama/","Мелодрама"},
		{"/kino/priklyucheniya/","Приключения"},
		{"/kino/semeynyy/","Семейный"},
		{"/kino/triller/","Триллер"},
		{"/kino/horrors/","Ужасы"},
		{"/kino/fantastic/","Фантастика"},
		{"/kino/fentezii/","Фэнтези"},
		{"/kino/2022/","2022 года"},
		{"/kino/kino-2021/","2021 года"},
		{"/kino/2020-gods/","2020 года"},
		{"/kino/otechestvennyy/","Русские"},
		{"/serials/","Сериалы - последние поступления"},
		{"/multiki/","Мультфильмы"},
		{"/animes/","Аниме"},
		{"/doc/","ТВ - шоу"},
		{"","ПОИСК"},
  ...
  if ret == 1 then
   ...
   elseif t[id].Name == 'Актёры' then
    ...
    page_kinokong('https://kinokong.sk' .. t[id].Action .. 'page/' .. 1 .. '/')
   end
  end
  if ret == 2 then
...
function kinokong()
	...
	local rc, answer = m_simpleTV.Http.Request(session, {url = 'https://kinokong.sk/podborka.html'})
		...
		local t,i={},1
			for w in answer:gmatch('<div class="podborki%-item%-block">.-</div>') do
				...
				t[i] = {}
				t[i].Id = i
				t[i].InfoPanelLogo = 'https://kinokong.sk/' .. logo
				t[i].Name = name:gsub('%&quot%;','"')
				t[i].Address = 'https://kinokong.sk' .. adr
...
function kinokong_info(url)
	...
	poster = 'https://kinokong.sk/' .. (poster or 'favicon.ico')
	...
	local t1,j={},2
		...
		t1[j]={}
		t1[j].Id = j
		t1[j].Address = 'https://kinokong.sk/index.php?do=actors&actor=' .. escape(m_simpleTV.Common.string_fromUTF8(adr))
		...
		t1[j]={}
		t1[j].Id = j
		t1[j].Address = 'https://kinokong.sk/index.php?do=directors&director=' .. escape(m_simpleTV.Common.string_fromUTF8(adr))
...
--	m_simpleTV.Http.Close(session)
	...
	local t, i = {}, 1
			...
			local logo, adr, name = w:match('<img src="(.-)".-<a href="(.-)" title="(.-)"')
				...
				t[i] = {}
				t[i].Id = i
				t[i].Address = 'https://kinokong.sk/index.php?do=actors&actor=' .. escape(m_simpleTV.Common.string_fromUTF8(m_simpleTV.Common.fromPercentEncoding(name:gsub(' /.+', ''):gsub(' ', '_'))))
				t[i].Name = name:gsub(' /.+', '')
