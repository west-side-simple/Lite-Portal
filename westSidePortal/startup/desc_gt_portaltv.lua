-- Видеоскрипт для стены лого медиапортала TV (15.05.2025)
	...
	m_simpleTV.User.TVPortal.stena_tvportal_type = getConfigVal('tvportal_type') or 'square'
	m_simpleTV.User.TVPortal.stena_tvportal_logopack = getConfigVal('tvportal_logopack') or 'original'
...
local function rand_color()
	local in_color = {
	ARGB(255, 197, 208, 230),
	ARGB(255, 119, 221, 231),
	ARGB(255, 205, 164, 222),
	ARGB(255, 193, 84, 193),
	ARGB(255, 128, 218, 235),
	ARGB(255, 255, 67, 164),
	ARGB(255, 197, 227, 132),
	ARGB(255, 255, 174, 66),
	ARGB(255, 17, 100, 180),
	ARGB(255, 41, 171, 135),
	ARGB(255, 93, 118, 203),
	...
	return in_color[math.random(#in_color)]
end
...
--				t.boundWidth = 1000
				...
				t.transparency = 200
				t.zorder=2
				AddElement(t,'TVPORTAL_STENA_BANNER1_BACK_IMG_ID')
...
--				t.boundWidth = 1000
				...
				AddElement(t,'ID_DIV_STENA_1')
					...
					t.backroundcorner = 2*2
					t.borderround = 2
					t.mousePressEventFunction = 'start_tv_d'
					AddElement(t,'ID_DIV_STENA_1')
...
--				t.boundWidth = 1000
				...
				t.transparency = 200
				t.zorder=2
				AddElement(t,'TVPORTAL_STENA_BANNER2_BACK_IMG_ID')
...
----------------------
			...
			for j = 1,#m_simpleTV.User.TVPortal.Channel_Of_Group_stena do
				...
				t.transparency = 200
				 ...
				 t.cursorShape = 13
				 t.bordercolor_UnderMouse = 0xFF4169E1
				if (tonumber(m_simpleTV.User.TVPortal.stena_tvportal_page) - 1) * 60 + j == tonumber(m_simpleTV.User.TVPortal.Channel_id) then
				...
				if m_simpleTV.User.TVPortal.stena_tvportal_type == 'square' then
					...
					nx = j - (math.ceil(j/10) - 1)*10
					ny = math.ceil(j/10)
					t.left = nx*dx - 240 + 8 + 48
					t.cx=178
					t.cy=98
					t.top  = ny*dy + 155 + 40 + 15
					t.imagepath = GetNames(m_simpleTV.User.TVPortal.Channel_Of_Group_stena[j].Name:gsub('^.-: ',''):gsub('^.-: ',''):gsub('\r',''):gsub(' ⟲.-$',''):gsub(' $',''):gsub(' orig$',''):gsub(' 50$',''))
				end
				t.class="IMAGE"
				t.minresx=-1
				...
				t.font_height = -6 / m_simpleTV.Interface.GetScale()*1.5
...
function change_stena_tvportal_type()
	...
	if m_simpleTV.User.TVPortal.stena_tvportal_type == 'square' then
		...
		setConfigVal('tvportal_type','vod')
	else
		m_simpleTV.User.TVPortal.stena_tvportal_type = 'square'
		setConfigVal('tvportal_type','square')
	end
...
function change_stena_tvportal_logopack()
	...
	if m_simpleTV.User.TVPortal.stena_tvportal_logopack == t1[12] then
		m_simpleTV.User.TVPortal.stena_tvportal_logopack = t1[1]
		m_simpleTV.User.TVPortal.stena_tvportal_type = 'square'
		setConfigVal('tvportal_logopack',t1[1])
		setConfigVal('tvportal_type','square')
		return stena_desc_tvportal_content()
	end
	for i = 1,#t1 do
		if m_simpleTV.User.TVPortal.stena_tvportal_logopack == t1[i] then
			...
			setConfigVal('tvportal_logopack',t1[i + 1])
			return stena_desc_tvportal_content()
		end
	end
end
