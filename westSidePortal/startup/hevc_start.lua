-- Плагин для трекера RipsClub (HEVC) lite portal 01.04.25
-- author west_side
...
function start_hevc()
	...
	local tt = {
		{"","TOP"},
		{"https://rips.club/search.php?part=1&year=&cat=0&standard=0&bit=0&order=0&search=&page=1","Фильмы"},
		{"https://rips.club/search.php?part=2&year=&cat=0&standard=0&bit=0&order=1&search=&page=1","Сериалы"},
		{"https://rips.club/search.php?part=3&year=&cat=0&standard=0&bit=0&order=1&search=&page=1","Коллекции"},
		{"https://rips.club/search.php?part=4&year=&cat=0&standard=0&bit=0&order=1&search=&page=1","Концерты"},
		{"https://rips.club/search.php?part=0&year=&cat=15&standard=0&bit=0&order=1&search=&page=1","Мультфильмы"},
		{"","ПОИСК"},
		}
		local t0={}
		for i=1,#tt do
...
function search_hevc(url)
	...
	local rc, answer = m_simpleTV.Http.Request(session, {url = url})
		...
		local t,i={},1
			for w in answer:gmatch('<article><h2>.-</article>') do
				local name = w:match('title="Подробнее">(.-)<')
				local year = name:match('%((%d+)') or 0
				local adr = w:match('href="(.-)"')
				local logo = w:match('src="(.-)"')
				local overview = w:match('<div class="side_cmprs">(.-)<div class="bottom">') or ''
		...
		local prev, nextt
		if tonumber(current) < tonumber(all) then
		nextt = url:gsub('page=' .. current,'page=' .. tonumber(current)+1)
		end
		if tonumber(current) == 1 and not url:match('page=(%d+)') then
		nextt = url .. '&page=2'
		end
--		debug_in_file(nextt .. '\n','c://1/nextt.txt')
...
function content_hevc(url)
	...
	local retAdr = answer:match('rel="nofollow" href="(magnet.-)"')
	...
	local t1,j={},2
		...
		t1[j]={}
		t1[j].Id = j
		t1[j].Address = 'https://rips.club/search.php?part=0&year=&cat=' .. adr .. '&standard=0&bit=0&order=0&search=&page=1'
		t1[j].Name = name
		j=j+1
		end
		end
