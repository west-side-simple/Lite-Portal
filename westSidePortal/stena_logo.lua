-- Видеоскрипт для формирования стены лого медиапортала TV выбранного фильтра - дополнение меню фильтра (18.06.25)
...
function select_mac(id, NameForId, adr, desc, ExtFilterID, portal, session, play)
	local t,i = {},1
	local activ_mac = desc:match('<font.->%[.-(%S%S:%S%S:%S%S:%S%S:%S%S:%S%S)')
	local activ_index = 1
	...
	if ret == 1 then
		...
		for i = 1,#t do
			...
			if tonumber(id1) == i then
				str = str .. '<font color=#0000ff>[' .. t[i].Name .. str1 .. ']</font>\n'
			else
...
--		debug_in_file(t[id1].Address .. '\n' .. 'type=stb&action=get_profile&hd=1&ver=ImageDescription: 0.2.18-r23-250; ImageDate: Wed Aug 29 10:49:53 EEST 2018; PORTAL version: 5.1.0; API Version: JS API version: 343; STB API version: 146; Player Engine version: 0x58c&num_banks=2&sn=' .. t[id1].Ser .. '&stb_type=MAG250&image_version=218&video_out=hdmi&device_id=' .. t[id1].Dev1 .. '&device_id2=' .. t[id1].Dev2 .. '&signature=&auth_second_step=1&hw_version=1.7-BD-00&not_valid_token=0&client_type=STB&hw_version_2=41b1b83de56bfcf125bc137cf91f03d5&timestamp='.. os.time() ..'&api_signature=263&metrics={"mac":"' .. t[id1].Name .. '","sn":"' .. t[id1].Ser .. '","model":"MAG250","type":"STB","uid":"","random":"' .. random_str .. '"}&JsHttpRequest=1-xml' .. '\n' .. answer .. '\n','c://1/answer_get_profile.txt')
		if rc~=200 then
...
function select_stena_logo(extFilterId, Filter_Name, play)
	...
	if channelId then
		m_simpleTV.OSD.ShowMessageT({text = 'Адрес плейлиста есть в PortalPLL', color = ARGB(255, 155, 155, 255)})
	else
		...
		if m_simpleTV.Database.ExecuteSql('INSERT INTO Channels ([Id],[ChannelOrder],[Name],[Logo],[Address],[ExtFilter],[TypeMedia]) VALUES (' .. channelId .. ',' .. channelId .. ',"' .. title .. '","' .. logo .. '","' .. 'portalTV_playlist=ID=' .. extFilterId .. '",' .. is_portal[1].Id ..',' .. 2 .. ');') then
			m_simpleTV.PlayList.Refresh()
			m_simpleTV.OSD.ShowMessageT({text = 'Адрес плейлиста добавлен в PortalPLL', color = ARGB(255, 155, 155, 255)})
		end
	end
	...
	if tonumber(m_simpleTV.Config.GetValue("Portal_TV_WS_upload_Enable","PortalTV.ini")) == 1 then
...
----------- отображение инфы
		...
		lfs.mkdir(folder)
		local folderWS = folder .. 'MyPlaylist/'
		lfs.mkdir(folderWS)
		local filePath = folderWS .. 'out' .. os.time() .. fileEnd
		debug_in_file(m3ustr, filePath)
		for j = 1, #t do
			local uservalue = {}
			uservalue.RawM3UString = ''
			if tonumber(t[j].StateCh) == 2 then
				if t[j].M3UCh:match('catchup=[\"\'](.-)[\"\']') then
					uservalue.RawM3UString = 'catchup="' .. t[j].M3UCh:match('catchup=[\"\'](.-)[\"\']') .. '" '
				end
				if t[j].M3UCh:match('catchup%-type=[\"\'](.-)[\"\']') then
					uservalue.RawM3UString = 'catchup-type=' .. t[j].M3UCh:match('catchup%-type=[\"\'](.-)[\"\']') .. '" '
				end
				if t[j].M3UCh:match('tvg%-rec=[\"\'](%d+)[\"\']') then
					uservalue.RawM3UString = uservalue.RawM3UString .. 'tvg-rec="' .. t[j].M3UCh:match('tvg%-rec=[\"\'](%d+)[\"\']') .. '" '
				end
				if t[j].M3UCh:match('catchup%-days=[\"\'](%d+)[\"\']') or t[j].M3UCh:match('catchup%-days=(%d+)') then
					uservalue.RawM3UString = uservalue.RawM3UString .. 'catchup-days="' .. (t[j].M3UCh:match('catchup%-days=[\"\'](%d+)[\"\']') or t[j].M3UCh:match('catchup%-days=(%d+)')) .. '" '
				end
			end
			uservalue.group = t[j].GroupCh
			uservalue.logo = t[j].LogoCh
			uservalue.name = t[j].NameCh
			uservalue.address = t[j].AddressCh
			m3ustr = '#EXTINF:-1 ' .. uservalue.RawM3UString .. 'group-title="' .. uservalue.group .. '" tvg-logo="' .. uservalue.logo .. '", ' .. uservalue.name .. '\n' .. uservalue.address .. '\n'
			debug_in_file(m3ustr, filePath)
			m_simpleTV.OSD.ShowMessageT({text = 'Выгрузка плейлиста: ' .. j .. ' из ' .. #t, color = ARGB(255, 155, 155, 255)})
		end

			ShowInfo('Плейлист сохранен в файл\n' .. 'out' .. os.time() .. fileEnd .. ' в папку\n' .. m_simpleTV.Common.multiByteToUTF8(folderWS))
