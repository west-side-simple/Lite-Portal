...
--------------------------------------------------
	...
	local function getApiKey()
		...
		if key then
			...
			if not key then
				m_simpleTV.OSD.ShowMessageT({imageParam = 'vSizeFactor="1.0" src="http://m24.do.am/images/logoport.png"', text = 'Youtube API Key not found', color = ARGB(255, 255, 255, 255), showTime = 1000 * 10})
...
local function get_info_for_video_channel(ch_id)
	...
	if rc ~= 200 then
		if getApiKey() then
			get_info_for_video_channel(ch_id)
		else
			return
		end
	end
...
function get_video_for_channel(ch_id, pageToken)
	...
	if rc ~= 200 then
		if getApiKey() then
			get_video_for_channel(ch_id, pageToken)
		else
			return
		end
	end
...
function get_video_for_playlist(pll_id, pageToken)
	...
	local rc, answer = m_simpleTV.Http.Request(session, {url = url, headers = 'Referer: https://www.youtube.com/tv'})
	m_simpleTV.Http.Close(session)
	if rc ~= 200 and rc~= 404 then
		if getApiKey() then
			get_video_for_playlist(pll_id, pageToken)
		else
			return
		end
	end
--	debug_in_file(pll_id .. '\n','c://1/testyoutube_pl.txt')
	require 'json'
	local tab = json.decode(answer:gsub('(%[%])', '"nil"'):gsub('\\n',''))
...
--	debug_in_file(ch_id .. '\n','c://1/testyoutube_chid.txt')
	...
	if rc ~= 200 then
		if getApiKey() then
			get_playlists_for_channel(ch_id, pageToken)
		else
			return
		end
	end
--	debug_in_file(answer .. '\n','c://1/testyoutube_pl.txt')
	...
	t[1].InfoPanelName = (t[1].InfoPanelName or '') .. ' | ' .. (m_simpleTV.User.TVPortal.stena_youtube_video_count or '') .. ' видео'
...
--				t.boundWidth = 1000
				...
				AddElement(t,'ID_DIV_STENA_2')
					...
					t.cy=0
					t.class="TEXT"
					t.text = 'video (' .. (m_simpleTV.User.TVPortal.stena_youtube_video_count or 0) .. ')'
					t.align = 0x0101
