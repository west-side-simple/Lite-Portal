--EX-FS portal - lite version west_side 02.11.21
...
local function setConfigVal(key,val)
	...
	t0.ExtButton1 = {ButtonEnable = true, ButtonName = ' TMDb '}
...
--	m_simpleTV.Http.Close(session)
	...
	local t1,j={},2
		...
		local ret, id = m_simpleTV.OSD.ShowSelect_UTF8(title, 0, t1, 30000, 1 + 4 + 8 + 2)
		if ret == 1 then
			if id == 1 then
			media_info(inAdr)
			else
