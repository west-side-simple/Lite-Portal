...
-- author west_side 17.11.24
	...
	if m_simpleTV.User.AudioWS and m_simpleTV.User.AudioWS.Use and tonumber(m_simpleTV.User.AudioWS.Use) == 1 then m_simpleTV.Control.EventPlayingInterval=20000 end
...
--	not m_simpleTV.Control.CurrentAddress:match('^http://.-8000/.+') and
	...
	if m_simpleTV.User.filmix then
		m_simpleTV.User.filmix.CurAddress = nil
	end
	if m_simpleTV.User.rezka then
		m_simpleTV.User.rezka.CurAddress = nil
	end
	if m_simpleTV.User.TMDB then
...
--				t[i].Address = 'webradio_network_id=' .. network_id .. '&id=' .. id .. '&track=' .. i .. '&' .. t[i].Address1
...
--				t[i].InfoPanelName = i .. '. ' .. t[i].Name
	...
	if network_id and id then
		...
		local __,id2 = m_simpleTV.OSD.ShowSelect_UTF8(m_simpleTV.User.AudioWS.Name, (m_simpleTV.User.AudioWS.track or 1) - 1, t2, 10000, 32)
		id2 = id2 or 1
		retAdr = GetTrackAdr()
