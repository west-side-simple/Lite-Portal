...
-- author west_side
	...
	then return end
	if not m_simpleTV.Control.CurrentAddress:match('/rc/filmix') and
	not m_simpleTV.Control.CurrentAddress:match('/lite/filmix')
	then return end
	local inAdr = m_simpleTV.Control.CurrentAddress:gsub('rjson=False&','')
	m_simpleTV.Control.ChangeAddress = 'Yes'
	m_simpleTV.Control.CurrentAddress = ''
	if not m_simpleTV.User then
		m_simpleTV.User = {}
	end
	if not m_simpleTV.User.FX then
		m_simpleTV.User.FX = {}
	end
	...
	m_simpleTV.User.FX.CurAddress = inAdr
	m_simpleTV.User.FX.DelayedAddress = nil
	m_simpleTV.User.TVPortal.balanser = 'FXLite'
	...
	local current_np = getConfigVal('perevod/fx') or ''
	if not getConfigVal('perevod/fx') then setConfigVal('perevod/fx','') end
		...
		for w in answer:gmatch('"season":.-%}%}%}%}') do
			...
			for v in w:gmatch('"episode":.-%}%}%}') do
				...
				if tonumber(kpid) == 77381 then
					...
					t[i].Address = m_simpleTV.User.FX.CurAddress:gsub('&s=%d+',''):gsub('&e=%d+',''):gsub('&kinopoisk_id=%d+','') .. '&kinopoisk_id=' .. t1[tonumber(s)][4] .. '&s=1&e=' .. e
					if t1[tonumber(s)][4] == tonumber(kpid_cur) and tonumber(episode) == tonumber(e) then current_ep = i end
				else
					t[i].Name = 'Сезон ' .. s .. ', Эпизод ' .. e
					t[i].Address = m_simpleTV.User.FX.CurAddress:gsub('&s=%d+',''):gsub('&e=%d+','') .. '&s=' .. s .. '&e=' .. e
...
--				debug_in_file(t[i].Name .. ' / ' .. t[i].Address .. '\n','c://1/answer_als.txt')
	...
	local function FXIndex(t)
		local lastQuality = tonumber(m_simpleTV.Config.GetValue('fx_qlty') or 5000)
		local index = #t
			for i = 1, #t do
...
--[[			for qlty, adr in urls:gmatch('"(.-)":"(.-)"') do
		...
		m_simpleTV.User.FX.Tab = t
		local index = FXIndex(t)
	 return t[index].Address
	end

	local function GetFXTrAdr(adr)
		local session = m_simpleTV.Http.New('Mozilla/5.0 (Windows NT 10.0; Win64; x64; rv:91.0) Gecko/20100101 Firefox/91.0')
		if not session then	return end
...
--			debug_in_file(w .. '\n','c://1/ans_FX2.txt')
	...
	function perevod_FX()
	local t = m_simpleTV.User.FX.TabPerevod
		if not t then return end
		if #t > 0 then
		local current_p = 1
		for i = 1,#t do
		if t[i].Name == getConfigVal('perevod/fx') then current_p = i end
		i = i + 1
		end
			t.ExtButton1 = {ButtonEnable = true, ButtonName = ' ⚙ ', ButtonScript = 'Qlty_FX()'}
			local ret, id = m_simpleTV.OSD.ShowSelect_UTF8(' 🔊 Перевод ', current_p - 1, t, 5000, 1 + 4 + 8 + 2)
			if ret == 1 then
			setConfigVal('perevod/fx', t[id].Name)
			local episode = m_simpleTV.User.FX.CurAddress:match('&e=(%d+)') or ''
...
--			adr = GetFXAdr(adr)
			...
			m_simpleTV.Control.CurrentAddress = m_simpleTV.User.FX.CurAddress:gsub('&t=%d+','') .. '&t=' .. id-1
--			m_simpleTV.Control.SetNewAddressT({address = t[id].Address .. episode, position = m_simpleTV.Control.GetPosition()})
	...
	function Qlty_FX()
		...
		local t = m_simpleTV.User.FX.Tab
			if not t then return end
		local index = FXIndex(t)
--			t.ExtButton0 = {ButtonEnable = true, ButtonName = ' Кинопоиск ', ButtonScript = ''}
			if getConfigVal('perevod/fx') ~= '' and m_simpleTV.User.FX.TabPerevod and #m_simpleTV.User.FX.TabPerevod > 1 then
				t.ExtButton1 = {ButtonEnable = true, ButtonName = ' 🔊 ', ButtonScript = 'perevod_FX()'}
			end
		local ret, id = m_simpleTV.OSD.ShowSelect_UTF8('⚙ Качество', index - 1, t, 5000, 1 + 4 + 8 + 2)
		if m_simpleTV.User.FX.isVideo == false then
			if m_simpleTV.User.FX.DelayedAddress then
		...
		if ret == 1 then
			...
			m_simpleTV.Config.SetValue('fx_qlty', t[id].qlty)
		end
		if ret == 2 then
			m_simpleTV.Control.SetNewAddress('**' .. m_simpleTV.User.FX.kpid, m_simpleTV.Control.GetPosition())
		end
		if ret == 3 then
			perevod_FX()
		end
	...
	m_simpleTV.User.FX.kpid = kpid
	local id_imdb,title_v,year_v,tv
--	id_imdb = inAdr:match('kp%=(tt%d+)')
	local season,episode=m_simpleTV.User.FX.CurAddress:match('&s=(%d+).-&e=(%d+)')
...
-------------------------
	...
	m_simpleTV.User.FX.isVideo = nil
...
--	debug_in_file(answer .. '\n','c://1/ans_FX.txt')
	retAdr = answer
		if not retAdr then
			m_simpleTV.Control.ExecuteAction(37)
			m_simpleTV.OSD.ShowMessageT({imageParam = 'vSizeFactor="1.0" src="http://m24.do.am/images/logoport.png"', text = 'FX: Медиаконтент не доступен', color = ARGB(255, 255, 255, 255), showTime = 1000 * 10})
...
--				debug_in_file(w .. '\n','c://1/ans_FX1.txt')
				...
				t1[i].Address = file
				t1[i].Name = name
				if t1[i].Name and t1[i].Name:gsub('%-','') == getConfigVal('perevod/fx'):gsub('%-','') then
				current_p = i
				is_perevod = true
--				debug_in_file(file .. '/' .. name .. '\n','c://1/ans_FXX1.txt')
				end
--				debug_in_file(file .. '/' .. name .. '/' .. current_p .. '\n','c://1/ans_FX1.txt')
				i=i+1
			end
...
--				debug_in_file(name .. '\n' .. file .. '\n','c://1/ans_fx1.txt')				
				...
				if t1[i].Name == getConfigVal('perevod/fx') then current_p = i end
				i=i+1
			end
		end
		m_simpleTV.User.FX.TabPerevod = t1
	if i > 2 then
		if current_p then
...
--		debug_in_file(kpid .. ' ' .. season .. '/' .. episode .. ' - ' .. #t .. '/' .. current_ep .. '\n' .. retAdr1 .. '\n','c://1/ans_FX4.txt')
		...
		m_simpleTV.User.FX.titleTab = t

		t.ExtButton0 = {ButtonEnable = true, ButtonName = ' ⚙ ', ButtonScript = 'Qlty_FX()'}
		if getConfigVal('perevod/fx') ~= '' and m_simpleTV.User.FX.TabPerevod and #m_simpleTV.User.FX.TabPerevod > 1 then
			t.ExtButton1 = {ButtonEnable = true, ButtonName = ' 🔊 ', ButtonScript = 'perevod_FX()'}
		end
		...
		m_simpleTV.OSD.ShowSelect_UTF8('FX: ' .. (title:gsub('^.- %- ','') or ''), 0, t, 8000, 32 + 64 + 128)
	end
